% Calcula PSTH y RASTER de cada estimulo para UN SOLO CANAL(TUNGSTENO o NNx)
...
if canal_automatico == 1
    ...
    params_clus = readtable( ...
    horzcat(directorio_clus,'/parametros_clus.txt'), ...
    'Delimiter','\t');

    puerto_canal_custom = char(params_clus.puerto_canal_custom);
...
% Guardo
if guardar == 1
    print_png(1, directorio_clus, strcat('_',string(puerto_canal_custom),...
        '_clus-',num2str(clus), '_spike-shape_', string(round(thr)), 'uV'))
    print_pdf(2, directorio_clus, strcat('_',string(puerto_canal_custom),...
        '_clus-',num2str(clus),'_grilla_PSTH-RASTER', string(round(thr)), 'uV.pdf'))
    print_pdf(3, directorio_clus, strcat('_',string(puerto_canal_custom),...
        '_clus-',num2str(clus),'_INT_pasa-ALTOS', '.pdf'))
    print_pdf(4, directorio_clus, strcat('_',string(puerto_canal_custom),...
        '_clus-',num2str(clus),'_INT_pasa-BAJOS', '.pdf'))
    print_pdf(5, directorio_clus, strcat('_',string(puerto_canal_custom),...
        '_clus-',num2str(clus),'_CORR_pasa-ALTOS', '.pdf'))
    print_pdf(6, directorio_clus, strcat('_',string(puerto_canal_custom),...
        '_clus-',num2str(clus),'_CORR_pasa-BAJOS', '.pdf'))
end
